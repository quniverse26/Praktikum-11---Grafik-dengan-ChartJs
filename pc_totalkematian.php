...
while ($row = mysqli_fetch_array($kasus)) {
	...
	$query = mysqli_query($koneksi, "SELECT sum(total_kematian) as total_kematian FROM tb_covid where id_kasus='". $row['id_kasus']."'");
...
<head>
	<title> Pie Chart - Total Kematian </title>
		...
		var config = {
			...
			data: {
				datasets: [{
					...
					label: 'Presentase Total Kematian Covid-19'
				}],
